...
- [x] Tauri desktop app (lightweight native shell)
  ...
  - [ ] Windows Authenticode signing via Microsoft Artifact Signing (formerly Trusted Signing / Azure Code Signing)
    ...
    - [ ] Verify signed `.msi` with `signtool verify /pa /v` in CI; confirm Authenticode chain on a clean Windows VM. **Current blocker:** the first dispatched signed build failed at the Tauri bundling step with `failed to bundle project ` program not found ``, indicating the `signCommand` couldn't locate `signtool.exe` (or the Trusted Signing dlib helper) on `PATH` despite `SIGNTOOL_PATH` being set. Likely fixes: (a) point `bundle.windows.signCommand` at the absolute `SIGNTOOL_PATH` instead of relying on `PATH`, or (b) ensure the Microsoft Artifact Signing Client Tools winget install actually resolved before the `tauri-action` step ran, or (c) cache the signtool path resolution between steps.
  ...
  - [ ] Desktop distribution & downloads (publish signed builds for end users)
    - [x] `tauri-action` wired in `desktop-build.yml`: every push to `main` re-publishes (or updates) the GitHub Release tagged `desktop-v<version>` (where `<version>` is `apps/desktop/package.json`'s `version` field, read by the `get-version` job). Each push uploads platform bundles (`.msi`, `.dmg`, `.AppImage`/`.deb`/`.rpm`), updater `.sig` files, and the `latest.json` updater manifest. PRs and other branches build but do not publish.
    - [ ] **Trigger hardening (follow-up):** the current "publish-on-every-main-push" model means any unrelated change to `main` re-uploads identical artifacts under the same tag. Migrate `desktop-build.yml` to one of:
      ...
      - At minimum, add a guard step that skips the publish step when the tag already exists at the current `apps/desktop/package.json` version (prevents accidental re-publishes of an unchanged version).
    - [ ] **First public release version:** bump `apps/desktop/package.json` to `1.0.0` (and `apps/desktop/src-tauri/tauri.conf.json` `version` to match) only as part of the dedicated release commit. Tag the release commit `desktop-v1.0.0` and (post trigger-hardening) push the tag to fire the release workflow. Until then, leave the version at `0.1.0` so the existing publish-on-push flow keeps producing `desktop-v0.1.0` rolling builds for internal verification.
