...
- [ ] **Slice 3b — bridge feed publisher** (deferred follow-up PR): unauthenticated read-only `/feed` WebSocket endpoint on the game server + bridge subscriber that posts world-scope envelopes to the configured Discord channel.
  - [ ] Server: second WS route at `/feed` keyed by `req.url`; per-connection `feedSubscribers: Set<WebSocket>` separate from the gameplay `sessions` Map; no `PlayerSession`, no command handler. Inbound messages on `/feed` are dropped or closed with code 1003 to enforce read-only by construction.
